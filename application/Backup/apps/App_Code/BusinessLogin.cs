...
public class BusinessLogin
{
    ...
    public void SendOTPSms(string username, string pin)
    {
        try
        {
            ...
            string notificationStatus = dt.Rows[0]["OTPDeliveryType"].ToString();
            if(notificationStatus == "1")
            {
                if (phone.Equals(""))
                {
                    throw new Exception("OTP COUDN'T BE SENT BECAUSE USER HAS NO REGISTERED PHONE, OR REGISTERED PHONE IN WRONG FORMAT.");
                }
                else
                {
            ...
            else if(notificationStatus == "2")
            {
                if (email.Equals(""))
                {
                    throw new Exception("OTP COUDN'T BE SENT BECAUSE USER HAS NO REGISTERED EMAIL, OR REGISTERED EMAIL IN WRONG FORMAT.");
                }
                else
                {
                    string subject = "PEGPAY ONE TIME PASSWORD";
                    string[] to = { email };
                    string[] cc = { email };
                    ArrayList list = new ArrayList();
                    SendMailNew(to, cc, Message, subject, list, "verification@pegasus.co.ug");
                }
            }
            else
            {
                ...
                {
                    throw new Exception("OTP COUDN'T BE SENT BECAUSE USER HAS NO REGISTERED EMAIL OR PHONE.");
                }
                else
                {
                    if (!phone.Equals(""))
    ...
    public void SendMailNew(string[] to, string[] cc, string body, string subj, ArrayList attachments,string fromEmail)
    {
        try
        {
            //BUILD EMAIL
            ...
            int smtpPort = 0;
            DataTable dt = datafile.GetEmailDetails(fromEmail);
            if(dt.Rows.Count>0)
            {
                DataRow dr = dt.Rows[0];
                smtpServer = dr["SMTPServer"].ToString();
                smtpPassword = dr["EmailPassword"].ToString();
                smtpPort = int.Parse(dr["Port"].ToString());
            ...
            {
                throw new Exception("NO SENDER EMAIL DETAILS SET");
            }
        }
        catch (Exception ex)
        {
            DataLogin df = new DataLogin();
            df.LogError("login", to[0], "", "0", ex.Message, "100");
            throw ex;
        }
